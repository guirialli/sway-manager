...
class ThemeService:
    ...
    def toggle(cls):
        ...
        # Apply Qt themes (qt5ct, qt6ct, Kvantum, KDE)
        cls._apply_qt_theme(new_theme)
    ...
    @classmethod
    def _update_ini_setting(cls, filepath: str, section: str, key: str, value: str):
        try:
    ...
    @classmethod
    def _apply_qt_theme(cls, new_theme: str):
        ...
        for conf_dir in ["~/.config/qt5ct", "~/.config/qt6ct"]:
            ...
            if os.path.isdir(conf_dir_expanded) or os.path.isfile(conf_path) or shutil.which(tool_name):
                cls._update_ini_setting(conf_path, "Appearance", "style", qt_style)
        ...
        if os.path.isdir(kv_dir) or os.path.isfile(kv_path) or shutil.which("kvantummanager"):
            cls._update_ini_setting(kv_path, "General", "theme", kv_theme)
