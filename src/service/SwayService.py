import subprocess
import re
import os
from utils.exceptions import SwayException


class SwayService:
    @classmethod
    def get_monitores(cls) -> list[str]:
        cmd = 'swaymsg -t get_outputs | grep "\\"name\\":"'
        processo = subprocess.run(cmd, shell=True, text=True, capture_output=True)
        monitores = re.findall(r'"name":\s*"([^"]+)"', processo.stdout)

        return monitores

    @classmethod
    def validar_monitores(cls):
        if len(cls.get_monitores()) < 2:
            raise ValueError("Apenas um monitor encontrado!")

    @classmethod
    def recarregar(cls):
        subprocess.run(["swaymsg", "reload"])

    @classmethod
    def escrever_arquivo(
        cls, conteudo: str, arquivo: str, append=False, pasta="~/.config/sway/config.d/"
    ):
        pasta = os.path.expanduser(pasta)
        arquivo_config = os.path.join(pasta, arquivo)
        try:
            os.makedirs(os.path.dirname(arquivo_config), exist_ok=True)

            with open(arquivo_config, "w" if not append else "a") as f:
                f.write(conteudo)

            cls.recarregar()
        except Exception as e:
            raise SwayException(
                f"Não foi possivel escrever no arquivo {arquivo_config}: {e}"
            )

    @classmethod
    def set_wallpaper(cls, caminho_imagem: str):
        """Define o wallpaper no Sway e atualiza o symlink ~/.config/sway/wallpaper"""
        if not os.path.isfile(caminho_imagem):
            raise SwayException(f"Arquivo de imagem não encontrado: {caminho_imagem}")

        conteudo = f'output "*" bg "{caminho_imagem}" fill\n'
        cls.escrever_arquivo(arquivo="42-wallpaper", conteudo=conteudo)

        sway_wp_link = os.path.expanduser("~/.config/sway/wallpaper")
        try:
            sway_dir = os.path.dirname(sway_wp_link)
            os.makedirs(sway_dir, exist_ok=True)
            if os.path.islink(sway_wp_link) or os.path.exists(sway_wp_link):
                os.remove(sway_wp_link)
            os.symlink(caminho_imagem, sway_wp_link)
        except Exception as e:
            print(f"Aviso ao atualizar symlink ~/.config/sway/wallpaper: {e}")

    @classmethod
    def get_current_wallpaper(cls) -> str | None:
        """Obtém o caminho do wallpaper atual do Sway (via symlink ou parsing de configurações)."""
        # 1. Verificar se o link/arquivo ~/.config/sway/wallpaper existe
        current_wp = os.path.expanduser("~/.config/sway/wallpaper")
        if os.path.islink(current_wp):
            try:
                target = os.readlink(current_wp)
                if os.path.isfile(target):
                    return target
            except Exception:
                pass
        elif os.path.isfile(current_wp):
            return current_wp

        # 2. Tentar ler de ~/.config/sway/config.d/42-wallpaper
        wp_config_file = os.path.expanduser("~/.config/sway/config.d/42-wallpaper")
        if os.path.isfile(wp_config_file):
            try:
                with open(wp_config_file, "r") as f:
                    content = f.read()
                match = re.search(r'bg\s+(?:"([^"]+)"|\'([^\']+)\'|(\S+))', content)
                if match:
                    path = match.group(1) or match.group(2) or match.group(3)
                    if path and os.path.isfile(path):
                        return path
            except Exception as e:
                print(f"Erro ao ler 42-wallpaper: {e}")

        # 3. Tentar procurar por qualquer arquivo em ~/.config/sway/config.d/ que defina bg
        config_d = os.path.expanduser("~/.config/sway/config.d")
        if os.path.isdir(config_d):
            for file_name in sorted(os.listdir(config_d)):
                file_path = os.path.join(config_d, file_name)
                if os.path.isfile(file_path):
                    try:
                        with open(file_path, "r") as f:
                            content = f.read()
                        match = re.search(r'bg\s+(?:"([^"]+)"|\'([^\']+)\'|(\S+))', content)
                        if match:
                            path = match.group(1) or match.group(2) or match.group(3)
                            if path and os.path.isfile(path):
                                return path
                    except Exception:
                        pass

        return None

